...
export function Jbro(props) {
  const group = useRef()

  // Works in dev ("/") and on GitHub Pages ("/jsmunasinghe-portfolio/")
  const withBase = (p) => `${import.meta.env.BASE_URL}${p.replace(/^\/+/, '')}`

  const glbUrl = withBase('models/jbro-transformed.glb')
  const fbxUrl = withBase('models/Wave Hip Hop Dance.fbx') // (rename to remove spaces if you can)
...
// Preload with the same BASE_URL-aware path
useGLTF.preload(`${import.meta.env.BASE_URL}models/jbro-transformed.glb`)
